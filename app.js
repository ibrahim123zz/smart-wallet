// بيانات الربط مع Supabase (سنغيرها بعد قليل)
...
async function loadData() {
    // 1. جلب الرصيد الحالي
    let { data: wallet, error } = await supabase.from('wallet').select('balance').single();
    if (wallet) {
        balanceEl.innerText = wallet.balance;
    }

    // 2. جلب سجل العمليات
    let { data: history, error2 } = await supabase.from('history').select('*').order('id', { ascending: false });
    if (history) {
...
async function updateTransaction(type, amount) {
    ...
    let newBalance = type === 'دخل' ? currentBalance + amount : currentBalance - amount;

    // تحديث الرصيد في جدول wallet
    await supabase.from('wallet').update({ balance: newBalance }).eq('id', 1);

    // إضافة العملية في جدول history
    await supabase.from('history').insert([{ type: type, amount: amount }]);
